...
class PhoneBookEntry {
 public:
     ...
     std::map<std::string,std::string> additionalPhones; // facoltativo infatti non viene messo nel costruttore
     std::string email;
};
...
class PhoneBook {
	public:
	   ...
	   void printAll(){
        //size() è metodo di vector che dà la lunghezza
		for(size_t i=0;i<entries.size();i++) entries[i].print();
	   }
       // il terzo argomento dice dove mettere l'out put. quindi sto sovrascrivendo
	   ...
	   void load() {
		  ...
		  if (myfile_in.is_open())
		  {
            ...
            // passo una reference field che lui riempie e poi facendo
            // push back passo all'elemento successivo
